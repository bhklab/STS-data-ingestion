...
DEFAULT_DATA_DIR = Path("extraction/data/proc/preclinical/GCSI")
DEFAULT_DATASET_NAME = "GCSI"
...
def gcsi_log2_tpm_plus_pseudocount_to_tpm(value: float | None) -> float | None:
    """Convert GCSI RNA-seq log2(TPM + 0.001) values back to TPM."""
    if value is None:
        return None
...
MOLECULAR_LOAD_PLAN = (
    {
        ...
        "value_transform": gcsi_log2_tpm_plus_pseudocount_to_tpm,
    },
    {
...
def read_csv(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Missing required GCSI CSV: {path}")
...
def ensure_prefixed_sample_ids(sample_ids: list[Any], *, auto_prefix: bool) -> list[str | None]:
    ...
    if unprefixed:
        ...
        raise ValueError(
            "GCSI sample IDs must already be prefixed before database insert. "
...
def seed_cell_lines(session: Session, *, dataset_id: int, data_dir: Path) -> dict[str, int]:
    ...
    print(f"Seeded GCSI cell lines: {len(lookup)}")
    return lookup
...
def seed_samples(
    ...
    *,
    ...
    if missing_cell_lines:
        ...
        raise ValueError(
            "Some GCSI sample rows reference cell lines that were not loaded into "
    ...
    print(f"Seeded GCSI samples: {len(rows)}")
    return sample_ids
...
def seed_treatment_response(
    ...
    *,
    ...
    if missing_cell_lines:
        preview = ", ".join(missing_cell_lines[:20])
        raise ValueError(
            "Some GCSI treatment-response rows reference cell lines that were not "
    ...
    print(f"Seeded GCSI treatment responses: {len(rows)}")
...
def seed_genes(session: Session, *, data_dir: Path) -> set[str]:
    ...
    print(f"Seeded/updated GCSI genes: {len(gene_ids)}")
    return gene_ids
...
def validate_molecular_samples_prefixed(sample_ids: set[str], *, label: str) -> None:
    ...
    if unprefixed:
        preview = ", ".join(unprefixed[:10])
        raise ValueError(
            f"GCSI {label} contains unprefixed sample_id values. Examples: {preview}. "
    ...
    *,
    ...
    for chunk_index, chunk_df in enumerate(iter_csv_chunks(path, chunksize=chunksize), start=1):
        ...
        if missing_sample_ids:
            ...
            raise ValueError(
                f"GCSI {label} has sample_id values missing from pre_clinical_sample. "
                f"Examples: {preview}"
            )
        ...
        if missing_gene_ids:
            ...
            raise ValueError(
                f"GCSI {label} has gene_id values missing from pre_clinical_gene. "
                f"Examples: {preview}"
            )
        ...
        print(
            f"Seeded GCSI {label} chunk {chunk_index}: "
            f"{len(rows)} candidate rows"
        )

    print(
        f"Finished GCSI {label}: {total_insert_candidates} candidate rows. "
...
def seed_gcsi(
    *,
    ...
    print("Finished GCSI preclinical seeding. Mutation upload was skipped.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Seed the GCSI preclinical dataset from extracted CSVs. Mutation upload is intentionally skipped."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=DEFAULT_DATA_DIR,
        help=f"Directory containing GCSI extracted CSVs. Default: {DEFAULT_DATA_DIR}",
    )
    parser.add_argument(
        "--dataset-name",
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Delete existing GCSI dataset rows before reloading.",
    )
    parser.add_argument(
        "--auto-prefix-samples",
        action="store_true",
        help=(
            "Automatically add gcsi_ to sample IDs if the sample CSV is not already prefixed. "
