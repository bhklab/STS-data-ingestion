...
def ensure_prefixed_sample_ids(sample_ids: list[Any], *, auto_prefix: bool) -> list[str | None]:
    ...
    if unprefixed:
        ...
        raise ValueError(
            f"GCSI sample IDs must already be prefixed before database insert. "
...
def seed_samples(
    ...
    *,
    ...
    if missing_cell_lines:
        ...
        raise ValueError(
            f"Some GCSI sample rows reference cell lines that were not loaded into "
...
def seed_treatment_response(
    ...
    *,
    ...
    if missing_cell_lines:
        preview = ", ".join(missing_cell_lines[:20])
        raise ValueError(
            f"Some GCSI treatment-response rows reference cell lines that were not "
...
def seed_genes(session: Session, *, data_dir: Path) -> set[str]:
    ...
    print(
        f"Inserted new GCSI genes where absent; existing gene IDs were skipped. "
...
def seed_molecular_file(
    ...
    *,
    ...
    for chunk_index, chunk_df in enumerate(iter_csv_chunks(path, chunksize=chunksize), start=1):
        ...
        print(f"Seeded GCSI {label} chunk {chunk_index}: {len(rows)} candidate rows")
...
def seed_dataset(
    *,
    ...
    print(f"Finished GCSI preclinical seeding. Mutation upload was skipped.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=f"Seed the GCSI preclinical dataset from extracted CSVs. Mutation upload is intentionally skipped."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help=f"Delete existing GCSI dataset rows before reloading.",
    )
    parser.add_argument(
        "--auto-prefix-samples",
        action="store_true",
