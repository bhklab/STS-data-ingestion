...
def ensure_prefixed_sample_ids(sample_ids: list[Any], *, auto_prefix: bool) -> list[str | None]:
    ...
    if unprefixed:
        ...
        raise ValueError(
            f"CTRPv2 sample IDs must already be prefixed before database insert. "
...
def seed_samples(
    ...
    *,
    ...
    if missing_cell_lines:
        ...
        raise ValueError(
            f"Some CTRPv2 sample rows reference cell lines that were not loaded into "
...
def seed_treatment_response(
    ...
    *,
    ...
    if missing_cell_lines:
        preview = ", ".join(missing_cell_lines[:20])
        raise ValueError(
            f"Some CTRPv2 treatment-response rows reference cell lines that were not "
...
def seed_genes(session: Session, *, data_dir: Path) -> set[str]:
    ...
    print(
        f"Inserted new CTRPv2 genes where absent; existing gene IDs were skipped. "
...
def validate_molecular_samples_prefixed(sample_ids: set[str], *, label: str) -> None:
    ...
    if unprefixed:
        preview = ", ".join(unprefixed[:10])
        raise ValueError(
            f"CTRPv2 {label} contains unprefixed sample_id values. Examples: {preview}. "
    ...
    *,
    ...
    for chunk_index, chunk_df in enumerate(iter_csv_chunks(path, chunksize=chunksize), start=1):
        ...
        if missing_sample_ids:
            ...
            raise ValueError(
                f"CTRPv2 {label} has sample_id values missing from pre_clinical_sample. "
                f"Examples: {preview}"
            )
        ...
        if missing_gene_ids:
            ...
            raise ValueError(
                f"CTRPv2 {label} has gene_id values missing from pre_clinical_gene. "
                f"Examples: {preview}"
            )
        ...
        print(f"Seeded CTRPv2 {label} chunk {chunk_index}: {len(rows)} candidate rows")

    print(
        f"Finished CTRPv2 {label}: {total_insert_candidates} candidate rows. "
...
def seed_dataset(
    *,
    ...
    with Session(engine) as session:
        if replace:
            print(f"Replacing existing dataset rows for {CTRPv2}")
    ...
    print(f"Finished CTRPv2 preclinical seeding. Mutation upload was skipped.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=f"Seed the CTRPv2 preclinical dataset from extracted CSVs. Mutation upload is intentionally skipped."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help=f"Delete existing CTRPv2 dataset rows before reloading.",
    )
    parser.add_argument(
        "--auto-prefix-samples",
        action="store_true",
