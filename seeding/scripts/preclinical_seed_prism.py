...
def ensure_prefixed_sample_ids(sample_ids: list[str], *, auto_prefix: bool) -> list[str]:
    ...
    if unprefixed:
        ...
        raise ValueError(
            "PRISM sample IDs must already be prefixed before database insert. "
...
def seed_samples(
    ...
    *,
    ...
    if missing_cell_lines:
        ...
        raise ValueError(
            "Some PRISM sample rows reference cell lines that were not loaded into "
...
def seed_treatment_response(
    ...
    *,
    ...
    if missing_cell_lines:
        preview = ", ".join(missing_cell_lines[:20])
        raise ValueError(
            "Some PRISM treatment-response rows reference cell lines that were not "
...
def seed_prism(
    *,
    ...
    print("Finished PRISM preclinical seeding.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Seed the PRISM preclinical dataset from extracted CSVs."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Delete existing PRISM dataset rows before reloading.",
    )
    parser.add_argument(
        "--auto-prefix-samples",
        action="store_true",
        help=(
            "Automatically add PRISM_ to sample IDs if the sample CSV is not already prefixed. "
